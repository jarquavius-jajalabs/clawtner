import { Env, generateId, now } from '../../lib/db';
...
function calculateNextFire(schedule: any): number {
  ...
  // Simple: set to today at the specified time
  ...
  if (today.getTime() <= nowMs) {
    if (schedule.type === 'daily') {
      ...
      if (!nextDay) nextDay = days[0] + 7; // wrap to next week
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  await context.env.CLAWTNER_DB.prepare(
    ...
    body.prompt_context || null,
    // HARDCODED: auto_approve is ALWAYS 0. Every message must be manually approved.
    // This is a safety feature - automated messages can ruin relationships.
    0,
    nextFire
  ).run();
